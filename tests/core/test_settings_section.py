...
class TestSettingsSection:
    ...
    def test_try_get_value_not_specifying_type(self, mocker: MockerFixture) -> None:
        expected_path = "logging.log_level.default"
        expected_value = "WARNING"
        settings_root_mock = mocker.create_autospec(SettingsRoot, instance=True)
        settings_root_mock.try_get_value.return_value = expected_value
        section = SettingsSection(root=settings_root_mock, path="logging.log_level")

        setting_value = section.try_get_value("default")

        assert setting_value == expected_value
        settings_root_mock.try_get_value.assert_called_once_with(expected_path, str)
    ...
    def test_try_get_value_specifying_type[TField](
        self,
        expected_path: str,
        ...
        setting_value = section.try_get_value("default", value_type)

        assert setting_value == expected_value
        settings_root_mock.try_get_value.assert_called_once_with(
            expected_path, value_type
        )

    def test_try_get_value_for_child_key_not_specifying_type(
        ...
        setting_value = section.try_get_value("log_level.default")
    ...
    def test_try_get_value_for_child_key_specifying_type(
        self, mocker: MockerFixture
    ) -> None:
        expected_value = 1
        settings_root_mock = mocker.create_autospec(SettingsRoot, instance=True)
        ...
        setting_value = section.try_get_value("log_level.default", int)
